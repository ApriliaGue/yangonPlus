<div class="flex items-center space-x-6 text-base font-medium text-black mb-4">
    <label class="flex items-center space-x-2 cursor-pointer">
        <input type="radio" name="trip-type" id="one-way" class="w-5 h-5" checked onchange="toggleTripType()">
        <span>One Way</span>
    </label>
    <label class="flex items-center space-x-2 cursor-pointer">
        <input type="radio" name="trip-type" id="round-trip" class="w-5 h-5" onchange="toggleTripType()">
...
<div class="flex flex-row flex-wrap items-end gap-y-4 gap-x-4 w-full">    
    <div class="w-full lg:flex-1">
        ...
        <input type="text" id="flight-name" name="flight-name" placeholder="MAI/MNA" class="w-full bg-white rounded-lg px-4 py-3 mt-1 text-base font-normal text-[#1F0730] shadow-sm focus:outline-none focus:ring-1 focus:ring-black">
    </div>
    <div class="w-[calc(50%-0.5rem)] lg:flex-1 relative">
        ...
        <div class="relative mt-1">
            ...
            <input type="text" id="departure-city" name="departure-city" placeholder="Departure city" class="w-full bg-white rounded-lg pl-10 pr-4 py-3 text-base font-normal text-[#1F0730] shadow-sm focus:outline-none focus:ring-1 focus:ring-black">
        </div>
        <button type="button" onclick="swapLocations()" aria-label="Swap Locations" class="absolute -right-6 top-1/2 -translate-y-1 z-20">
    ...
    <div class="w-[calc(50%-0.5rem)] lg:flex-1">
        ...
        <div class="relative mt-1">
            ...
            <input type="text" id="arrival-city" name="arrival-city" placeholder="Arrival city" class="w-full bg-white rounded-lg pl-10 pr-4 py-3 text-base font-normal text-[#1F0730] shadow-sm focus:outline-none focus:ring-1 focus:ring-black">
        </div>
    </div>
    <div id="departure-date-container" class="w-[calc(50%-0.5rem)] lg:flex-1 relative">
        ...
        <div class="relative mt-1">
            ...
            <input type="text" id="departure-date" name="departure-date" placeholder="Select date" onclick="document.getElementById('hidden-dep-picker').showPicker()" class="w-full bg-white rounded-lg pl-10 pr-4 py-3 text-base font-normal text-[#1F0730] shadow-sm focus:outline-none focus:ring-1 focus:ring-black" readonly>
            <input type="date" id="hidden-dep-picker" class="absolute inset-0 opacity-0 pointer-events-none" onchange="document.getElementById('departure-date').value = this.value">
        </div> 
        <button id="date-swap-btn" type="button" onclick="swapDates()" aria-label="Swap Dates" class="absolute -right-6 top-1/2 -translate-y-1 z-20 hidden">
            <svg width="35" height="36" viewBox="0 0 35 36" fill="none" xmlns="http://www.w3.org/2000/svg">
                <rect width="35" height="36" rx="17.3144" fill="#EBEDA4" />
                <path fill-rule="evenodd" clip-rule="evenodd" d="M11 21.5C11 21.7761 11.2239 22 11.5 22H23.2929L20.1464 25.1464C19.9512 25.3417 19.9512 25.6583 20.1464 25.8536C20.3417 26.0488 20.6583 26.0488 20.8536 25.8536L24.8536 21.8536C25.0488 21.6583 25.0488 21.3417 24.8536 21.1464L20.8536 17.1464C20.6583 16.9512 20.3417 16.9512 20.1464 17.1464C19.9512 17.3417 19.9512 17.6583 20.1464 17.8536L23.2929 21H11.5C11.2239 21 11 21.2239 11 21.5Z" fill="black" />
                <path fill-rule="evenodd" clip-rule="evenodd" d="M25 14.5C25 14.7761 24.7761 15 24.5 15H12.7071L15.8536 18.1464C16.0488 18.3417 16.0488 18.6583 15.8536 18.8536C15.6583 19.0488 15.3417 19.0488 15.1464 18.8536L11.1464 14.8536C10.9512 14.6583 10.9512 14.3417 11.1464 14.1464L15.1464 10.1464C15.3417 9.95118 15.6583 9.95118 15.8536 10.1464C16.0488 10.3417 16.0488 10.6583 15.8536 10.8536L12.7071 14H24.5C24.7761 14 25 14.2239 25 14.5Z" fill="black" />
    ...
    <div id="return-date-container" class="w-[calc(50%-0.5rem)] lg:flex-1 hidden">
        ...
        <div class="relative mt-1">
            ...
            <input type="text" id="return-date" name="return-date" placeholder="Select date" onclick="document.getElementById('hidden-ret-picker').showPicker()" class="w-full bg-white rounded-lg pl-10 pr-4 py-3 text-base font-normal text-[#1F0730] shadow-sm focus:outline-none focus:ring-1 focus:ring-black" readonly>
            <input type="date" id="hidden-ret-picker" class="absolute inset-0 opacity-0 pointer-events-none" onchange="document.getElementById('return-date').value = this.value">
        </div>
    </div>
</div>
...
<script>
    function toggleTripType() {
        ...
        const dateSwapBtn = document.getElementById('date-swap-btn');
        const departureContainer = document.getElementById('departure-date-container');

        if (isRoundTrip) {
            returnContainer.classList.remove('hidden');
            dateSwapBtn.classList.remove('hidden');
            // Keeps side-by-side calculation on mobile rows
            departureContainer.classList.remove('w-full');
            departureContainer.classList.add('w-[calc(50%-0.5rem)]');
        } else {
            returnContainer.classList.add('hidden');
            dateSwapBtn.classList.add('hidden');
            // Stretches out neatly if return container collapses out of view
            departureContainer.classList.remove('w-[calc(50%-0.5rem)]');
            departureContainer.classList.add('w-full', 'lg:w-auto');
        }
    }
    ...
    function swapDates() {
        ...
        const departurePicker = document.getElementById('hidden-dep-picker');
        const returnPicker = document.getElementById('hidden-ret-picker');

        // Swap visual fields
        const tempDate = departureDateInput.value;
        departureDateInput.value = returnDateInput.value;
        returnDateInput.value = tempDate;

        // Swap background hidden components
        const tempPicker = departurePicker.value;
        departurePicker.value = returnPicker.value;
        returnPicker.value = tempPicker;
    }
