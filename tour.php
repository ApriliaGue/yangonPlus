...
<div class="bg-[#F8F8F5]">
    <div id="tour" class="container-default py-10 md:py-18 scroll-mt-12">
        <div class="space-y-10 sm:space-y-16 lg:space-y-20">
            <div class="space-y-6">
                <!-- Cards -->
                <?php $active_tour = isset($_GET['tour']) ? $_GET['tour'] : 'mm'; ?>
                <div class="flex flex-wrap gap-2 text-sm sm:text-base font-medium">
                    <a href="?tour=mm#tour" class="inline-flex min-h-11 items-center justify-center px-4 py-2.5 rounded-full cursor-pointer transition-colors duration-200 select-none <?php echo ($active_tour === 'mm') ? 'bg-[#EFD64D] text-[#1F0730]' : 'bg-[#EFD64D1A] text-[#3E465F] hover:bg-[#EBE359]'; ?>">Myanmar Tours</a>
                    <a href="?tour=inter#tour" class="inline-flex min-h-11 items-center justify-center px-4 py-2.5 rounded-full cursor-pointer transition-colors duration-200 select-none <?php echo ($active_tour === 'inter') ? 'bg-[#EFD64D] text-[#1F0730]' : 'bg-[#EFD64D1A] text-[#3E465F] hover:bg-[#EBE359]'; ?>">International Tours</a>
                </div>
                <?php if ($active_tour === 'mm'): ?>
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
                        <?php for ($i = 0; $i < 8; $i++): ?>
                            <div class="card bg-white hover:shadow-lg rounded-2xl overflow-hidden transition-shadow duration-300">
                                <img src="assets/img/tour5.jpg" alt="" class="w-full h-56 sm:h-64 lg:h-72 xl:h-80 object-cover rounded-t-2xl">
                                <div class="p-4 sm:p-5">
                                    ...
                                    <h5 class="text-lg sm:text-xl font-semibold pb-2 text-[#1F0730] leading-snug">Yangon City Highlights Adventure</h5>
                                    <p class="min-h-0 lg:min-h-18 text-sm sm:text-base font-normal text-[#3E465F] leading-relaxed">Dive into the bustling life of Yangon with our curated city tour featuring iconic landmarks and local delicacies. Discover the heart of Myanmar!</p>
                                    <hr class="my-4 border-[#2E319280] opacity-50">
                                    <div class="flex items-center gap-3 text-center text-sm md:text-base font-medium text-[#1F0730]">
                                        <a href="tour-inquiry.php" class="w-1/2 border border-[#EFD64D] hover:bg-[#EFD64D] py-2.5 rounded-full transition duration-300">Inquiry</a>
                                        <a href="tour-detail.php" class="w-1/2 border border-[#EFD64D] bg-[#EFD64D] hover:bg-[#EBE359] py-2.5 rounded-full transition duration-300">Details</a>
                                    </div>
                                </div>
                            </div>
                        <?php endfor; ?>
                    ...
                    <!-- Pagination -->
                    <div class="mx-auto flex flex-wrap items-center justify-center gap-1 text-base sm:text-xl font-normal font-roboto">
                        <button type="button" class="flex h-10 w-10 sm:h-14 sm:w-14 items-center justify-center rounded-sm bg-[#EEEEEE] text-[#151567]">&laquo;</button>
                        <button type="button" class="flex h-10 w-10 sm:h-14 sm:w-14 items-center justify-center rounded-sm text-[#4F4F4F]">1</button>
                        <button type="button" class="flex h-10 w-10 sm:h-14 sm:w-14 items-center justify-center rounded-sm text-[#4F4F4F]">2</button>
                        <button type="button" class="flex h-10 w-10 sm:h-14 sm:w-14 items-center justify-center rounded-sm text-[#4F4F4F]">3</button>
                        <button type="button" class="flex h-10 w-10 sm:h-14 sm:w-14 items-center justify-center rounded-sm text-[#4F4F4F]">&raquo;</button>
                    </div>
                <?php endif; ?>
